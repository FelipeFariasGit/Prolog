...
classExpression(complementOf(Middle)) -->
	[In], {
	       atom_con2cat('ObjectComplementOf(',Rest,')',In),
	       classExpression(Middle, [Rest], [])
	      }.
...
subClassOf(Expression) -->
    [In], {
        ...
        complex_therm_sub(List, Expression), !
          }.
...
classExpression(A) -->
    [A], {
        ...
        \+atom_concat('ObjectMinCardinality', _, A), %antes havia ObjectMixtCardianlity. (?)
	...
	\+atom_concat('ObjectComplementOf', _,A),
...
%complex_therm([HeadIn|[]], Expression) :-
%	iri(Expression, [HeadIn], []).
...
complex_therm_sub([HeadIn], Expression) :-
	classExpression(Expression, [HeadIn], []).

complex_therm_sub([HeadIn|In], Expression) :-
	classExpression(Out, [HeadIn], []),
	complex_therm_sub(In, Out2),
...
complex_therm_sub([HeadIn|In], Expression):-
	...
	atom_concat(Temp, HeadIn2, HeadIn3),
	append([HeadIn3], Rest, In2),
	complex_therm_sub(In2, Expression).
